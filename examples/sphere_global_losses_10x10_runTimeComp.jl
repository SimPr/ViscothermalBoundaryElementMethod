...
function reconstructUnknowns(LGM::LossyGlobalOuter,sol::Vector{ComplexF64},M::Int64)
    ...
    v_n0 = LGM.Nd'*sol[4M+1:7M]; 
    v_t = sol[4M+1:7M] + LGM.Nd*v_n0; # Computing the tangential velocity by substracting the normal information
...
K = 1;
gr(size=(600,500));
scatter(ang_axis[1:K:end],real.(pa[1:K:end]),label="BEM - 10n Dense",marker=:cross,markersize=2,color=:black,dpi=400);
plot!(ang_axis[perm],real.(pasAN[perm]),label="Analytical",linewidth=1,color=:blue);
ylabel!("Re(Pa)");
title!("Frequency = $(freq) Hz");
xlabel!("Angle [deg]");
savefig("paGlobal10x10_$(M)DOFs_$(Int(freq))Hz.png")
...
if compute_full_solution == true
    ...
    savefig("allGlobal10x10_Real_$(M)DOFs_$(Int(freq))Hz.png")
    ...
    plt4 = plot(plt1,plt2,plt3,layout=(3,1))
    savefig("allGlobal10x10_Imag_$(M)DOFs_$(Int(freq))Hz.png")
    ...
    plt4 = plot(plt1,plt2,plt3,layout=(3,1))
    savefig("allGlobal10x10_Abs_$(M)DOFs_$(Int(freq))Hz.png")

    # Saving data
    jldsave("runtimes10x10_$(M)DOFs_$(Int(freq))Hz.JLD2", 
    runtimes=output,
    pa=pa,
    pasAN=pasAN,
    v_rAN_V=v_rAN_V,
