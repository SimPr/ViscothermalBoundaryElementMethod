...
# dense format
#LGM_dense  = BoundaryIntegralEquations._full1(LGM);
#cond(LGM_dense)
#pa = LGM_dense\rhs;
...
K = 1
gr(size=(600,500))
scatter(ang_axis[1:K:end],real.(pa[1:K:end]),label="BEM - 1n",marker=:cross,markersize=2,color=:black,dpi=400);
plot!(ang_axis[perm],real.(pasAN[perm]),label="Analytical",linewidth=1,color=:blue);
ylabel!("Re(Pa)");
title!("Frequency = $(freq) Hz");
xlabel!("Angle [deg]")
#savefig("paGlobal1x1_$(M)DOFs_$(freq)Hz.png")
#===========================================================================================
...
#savefig("allGlobal1x1_Real_$(M)DOFs_$(freq)Hz.png")
...
plt4 = plot(plt1,plt2,plt3,layout=(3,1))
#savefig("allGlobal1x1_Imag_$(M)DOFs_$(freq)Hz.png")
...
plt4 = plot(plt1,plt2,plt3,layout=(3,1))
#savefig("allGlobal1x1_Abs_$(M)DOFs_$(freq)Hz.png")
#===========================================================================================
                                   Same with FMM (plus comparison)
===========================================================================================#
# solution
LGM = LossyGlobalOuter(mesh,freq;fmm_on=true,depth=1,n=3);
rhs = LGM.Ga*gmres(LGM.inner,(LGM.Dr*v0 - LGM.Nd'*gmres(LGM.Gv,LGM.Hv*v0));verbose=true);
pa_fmm = gmres(LGM,rhs;verbose=true);
...
title!("Frequency = $(freq) Hz");
xlabel!("Angle [deg]")
#savefig("paGlobal1x1FMM_$(M)DOFs_$(freq)Hz.png")

# reconstruct unknowns
tmp1 =  gmres(LGM.Gh,LGM.Hh*pa_fmm;verbose=true)
dpa  = -gmres(LGM.Ga,LGM.Ha*pa_fmm;verbose=true) # <- This is the bottleneck...
v  = v0 - (LGM.mu_a*LGM.Dc*pa_fmm + LGM.mu_h*LGM.Nd*tmp1 + LGM.phi_a*LGM.Nd*dpa)
# Local components of the viscous velocity on the boundary
v_n0_fmm   = LGM.Nd'*v 
v_t = v + LGM.Nd*v_n0_fmm # Computing the tangential velocity by substracting the normal information
vt_sum_fmm = sqrt.(v_t[0M+1:1M].^2 + v_t[1M+1:2M].^2 + v_t[2M+1:3M].^2)
...
ylabel!("|Vn|"); plot!(ang_axis[perm],abs.(v_rAN_V[perm]),label="Analytical",linewidth=2,color=:blue); # inconsistency with analytical solution >> v_rAN_V pointing outwards
plt3 = scatter(ang_axis,abs.(vt_sum),label="BEM",marker=:cross,markersize=2,color=:black);
